...
type Props = { size?: number; tone?: string; shadow?: boolean };

export default function YSLogo({ size = 128, tone = '#ff6a00', shadow = true }: Props) {
  const s = size;
  return (
    <div
      aria-label="YS Logo"
      style={{
        width: s, height: s,
        filter: shadow ? 'drop-shadow(0 12px 30px rgba(255,106,0,.35))' : undefined,
        animation: 'ysPulse 6s ease-in-out infinite',
      }}
    >
      <svg viewBox="0 0 100 100" width={s} height={s} role="img">
        ...
        {/* Y estilizado */}
        <path d="M28 30 L42 48 L42 70"
              stroke={tone} strokeWidth="10" strokeLinecap="round" fill="none"/>
        {/* S estilizado */}
