...
const CalendarDayTile = memo(function CalendarDayTile({
  ...
  const containerBaseClass =
    'relative flex flex-col rounded-2xl border border-gray-200/70 bg-white shadow-[0_1px_0_rgba(0,0,0,0.02)] transition-shadow duration-150 hover:shadow-[0_6px_18px_rgba(15,23,42,0.06)]';
  const containerClassName = `${containerBaseClass} ${dimmed ? 'opacity-60' : 'opacity-100'}`;

  const badgeBaseClass = 'ml-auto text-[12px] font-semibold rounded-full px-2 py-0.5';
  const badgeClassName = `${badgeBaseClass} ${
    isToday
