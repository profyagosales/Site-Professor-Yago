...
export default function AnnotationEditorRich({
  value,
  onChange,
  currentPage,
  onSelect,
  ...
  const pageItems = useMemo(
    () => value.filter(a => a.page === currentPage),
    [value, currentPage]
  );
  ...
  function clearPage() {
    ...
    if (
      selectedId &&
      value.find(a => a.id === selectedId)?.page === currentPage
    )
      onSelect(null);
  }
    ...
    <div className='rounded-lg border border-[#E5E7EB]'>
      <div className='flex items-center justify-between border-b px-3 py-2'>
        ...
        <div className='text-xs text-ys-ink-2'>
          Pág. {currentPage} • {pageItems.length}
        </div>
      </div>
      <div className='flex items-center gap-2 px-3 py-2 text-xs'>
        ...
        >
          Próxima com anotação
        </button>
      </div>
      <div className='max-h-64 overflow-auto px-2 pb-2' ref={listRef}>
        {pageItems.length === 0 && (
          <div className='px-3 py-6 text-center text-xs text-ys-ink-2'>
        ...
        {pageItems.map(a => (
          <div
            key={a.id}
            data-id={a.id}
            className={`group mb-1 flex items-start justify-between rounded p-2 text-sm ${selectedId === a.id ? 'bg-[#FFF7ED]' : 'hover:bg-[#F3F4F6]'}`}
          ...
          >
            ...
            <button
              className='invisible ml-2 rounded border px-2 py-1 text-xs group-hover:visible'
              onClick={ev => {
                ev.stopPropagation();
                remove(a.id);
              }}
