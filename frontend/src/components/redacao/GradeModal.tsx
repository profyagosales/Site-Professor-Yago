...
export default function GradeModal({ open, onClose, essay, onGraded }: Props) {
  ...
  useEffect(() => {
    const greens = annotations.filter(
  ...
  async function submit() {
    const w = Number(weight);
    if (isNaN(w) || w < 0 || w > 10) {
      setErr('Peso bimestral inválido (0 a 10)');
      return;
    }
    if (essayType === 'ENEM' && !annul) {
      const arr = [c1, c2, c3, c4, c5].map(Number);
      if (arr.some(n => !validENEM.includes(n))) {
        setErr('Competências do ENEM devem ser 0,40,80,120,160 ou 200');
        return;
      }
      setErr(null);
      try {
    ...
    if (essayType === 'PAS') {
      const nc = Number(NC);
      const nl = Number(NL);
      if (isNaN(nc) || nc < 0 || nc > 10 || isNaN(nl) || nl < 1) {
        setErr('PAS: NC 0..10 e NL >= 1');
        return;
      }
      setErr(null);
      try {
    ...
    >
      <div className='w-full max-w-3xl rounded-2xl bg-white p-6 shadow-ys-lg'>
        <div className='mb-4 flex items-center justify-between'>
          ...
          <button className='text-ys-ink' onClick={onClose}>
            Fechar
          </button>
        </div>
          ...
          <div className='space-y-3'>
              ...
              <div className='space-y-2'>
                ...
                <div className='grid grid-cols-5 gap-2'>
                  {[
                    ...
                    <select
                      ...
                      className='rounded border border-[#E5E7EB] p-2'
                    >
                ...
                <label className='inline-flex items-center gap-2 text-sm text-[#111827]'>
                  <input
                    type='checkbox'
                    checked={annul}
                    onChange={e => setAnnul(e.target.checked)}
                  />{' '}
                  Anular
                </label>
              </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      const currentGreens = annotations.filter(
                        a =>
                          a.color === 'green' &&
                          a.label?.toLowerCase().includes('erro')
                      );
            ...
            <div className='flex items-center gap-4 text-sm text-[#111827]'>
              <label className='inline-flex items-center gap-2'>
                <input
                  type='checkbox'
                  checked={alsoRenderPdf}
                  onChange={e => setAlsoRenderPdf(e.target.checked)}
                />{' '}
                Gerar PDF
              </label>
              <label className='inline-flex items-center gap-2'>
          ...
          <div className='min-h-[320px] overflow-hidden rounded-lg border border-[#E5E7EB] bg-[#F9FAFB]'>
            {essay.fileUrl ? (
              isPdf ? (
                ...
                <a
                  className='block p-4 text-orange-600 underline'
                  href={essay.fileUrl}
                  target='_blank'
                  rel='noreferrer'
                >
              ...
              <div className='flex h-full items-center justify-center p-4 text-sm text-ys-ink-2'>
                Sem arquivo
              </div>
            )}
          </div>
