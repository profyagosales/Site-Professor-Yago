...
const names = ['Ana', 'Bruno', 'Carla', 'Diego', 'Elisa', 'Fabio', 'Gi', 'Henrique', 'Isa', 'João'];
const classes = ['1A', '1B', '2A', '2B', '3A'];

const all: Essay[] = Array.from({ length: 12 }).map((_, i) => ({
  id: String(i + 1),
...
export function getMockPage(params: { status: EssayStatus; page: number; pageSize: number; q?: string; classId?: string }) {
