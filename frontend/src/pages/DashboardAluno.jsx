...
function ShortcutCards() {
  const shortcuts = [
    ...(flags.redaction
      ? [
          {
            path: ROUTES.aluno.redacoes,
            title: 'Redações',
            subtitle: 'Minhas redações',
            icon: <FaPen className='text-orange w-6 h-6' />,
          },
        ]
      : []),
    {
      path: ROUTES.aluno.notas,
      title: 'Notas',
      subtitle: 'Resumo por bimestre',
      icon: <FaStar className='text-orange w-6 h-6' />,
    },
    {
      path: ROUTES.aluno.gabaritos,
      title: 'Gabaritos',
      subtitle: 'PDFs corrigidos',
      icon: <FaFilePdf className='text-orange w-6 h-6' />,
    },
    {
      path: ROUTES.aluno.caderno,
      title: 'Caderno',
      subtitle: 'Vistos do bimestre',
      icon: <FaBook className='text-orange w-6 h-6' />,
    },
  ];
  return (
  ...
  return (
    <div className='ys-card p-md'>
      <div className='flex justify-between items-center mb-sm'>
        ...
        <Link to={ROUTES.aluno.notas} className='text-orange text-sm'>
          Ver todos
        </Link>
      </div>
      <ul className='space-y-1 text-sm'>
        {exams.length === 0 && (
          <li className='text-black/60'>Nenhuma avaliação</li>
        )}
        {exams.map(e => (
...
function ContentsCard({ contents = [] }) {
  return (
    <div className='ys-card p-md'>
      ...
      <ul className='space-y-1 text-sm'>
        {contents.length === 0 && (
          <li className='text-black/60'>Nenhum conteúdo</li>
        )}
        {contents.map(c => (
...
function GradesBox({ grades }) {
  const activitiesScore =
    grades?.activities?.reduce((s, a) => s + (a.score || 0), 0) || 0;
  ...
  return (
    <div className='ys-card p-md'>
      <div className='flex justify-between items-center mb-sm'>
        ...
        <Link to={ROUTES.aluno.caderno} className='text-orange text-sm'>
          Ver detalhes
        </Link>
      </div>
      <p className='text-sm mb-sm'>Valor do caderno: {total}</p>
      <p className='text-sm mb-md'>
        Vistos: {seen}/{totalItems}
      </p>
      <ul className='space-y-1 text-sm'>
