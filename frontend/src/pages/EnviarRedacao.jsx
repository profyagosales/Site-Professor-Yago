  ...
  return (
    <div className='pt-20 p-md space-y-md'>
      ...
      <div className='space-y-sm'>
        ...
        {essays.length === 0 && (
          <p className='text-black/60'>Nenhuma redação enviada</p>
        )}
        {essays.map(e => (
          <div
            key={e._id || e.id}
            className='ys-card p-md flex justify-between items-center'
          >
            <div>
              <p className='font-medium'>
                {e.customTheme || e.theme?.name || 'Tema'}
              </p>
              <p className='text-sm text-black/70'>Bimestre: {e.bimester}</p>
            </div>
            <div className='text-right'>
              <p className='text-sm font-medium'>
                {e.status === 'GRADED' ? `Nota: ${e.scaledScore}` : 'Pendente'}
              </p>
