  ...
  return (
    <main className="min-h-[calc(100vh-64px)] flex items-center justify-center px-6 py-14">
      <div className="w-full max-w-md rounded-2xl bg-white/70 ring-1 ring-[var(--ring)] backdrop-blur-sm p-6 md:p-7 shadow-[0_1px_0_rgba(255,255,255,.5),0_12px_24px_rgba(15,23,42,.06)]">
        <h1 className="text-xl font-bold text-body">Login Aluno</h1>
        <p className="text-[13px] text-muted/90 mt-1">
        ...
        <form className="mt-6 space-y-4" onSubmit={submit}>
          <label className="block">
            ...
            <input
              className="mt-1 w-full rounded-lg border border-slate-300/70 bg-white px-3 py-2 text-[15px] outline-none focus:ring-2 focus:ring-[rgb(var(--brand))]"
              value={email} onChange={e=>setEmail(e.target.value)} type="email" required
            />
          </label>

          <label className="block">
            <span className="text-[13px] text-muted">Senha</span>
            <input
              className="mt-1 w-full rounded-lg border border-slate-300/70 bg-white px-3 py-2 text-[15px] outline-none focus:ring-2 focus:ring-[rgb(var(--brand))]"
