...
export function LoginProfessorPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!email.trim() || !password) {
      ...
      emailRef.current && !email.trim() && emailRef.current.focus()
      return
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 p-4">
      <div className="w-full max-w-md p-6 bg-white/80 backdrop-blur rounded-xl shadow-lg border border-slate-200">
        <h1 className="text-2xl font-bold text-center mb-2">Acesso do Professor</h1>
        ...
        <form className="space-y-5" onSubmit={handleSubmit} noValidate>
          <div className="space-y-1.5">
            ...
            <input
              ...
              onBlur={()=> { if (submittedRef.current && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email.trim())) setError('Formato de email inválido') }}
