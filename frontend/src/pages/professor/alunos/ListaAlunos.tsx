  ...
  return (
    <Page title="Alunos" subtitle="Buscar e visualizar perfis.">
      <div className="mb-3 flex items-center gap-2">
        <input value={q} onChange={e=>{ setQ(e.target.value); setPage(1); }} placeholder="Buscar aluno" className="rounded-lg border border-[#E5E7EB] p-2 focus:outline-none focus:ring-2 focus:ring-orange-500" />
      </div>
      {err && <p className="text-red-600 mb-2">{err}</p>}
      {loading ? <p>Carregando…</p> : (
        <div className="overflow-hidden rounded-xl border border-[#E5E7EB] bg-white">
          <table className="w-full text-sm text-[#111827]">
            <thead className="bg-[#F3F4F6] text-left text-[#374151]"><tr><th className="px-4 py-3">Aluno</th><th className="px-4 py-3">E-mail</th><th className="px-4 py-3">Ações</th></tr></thead>
            <tbody>
              {data.items.map((s:any) => (
                <tr key={s._id || s.id} className="odd:bg-[#F9FAFB]">
                  <td className="px-4 py-3">{s.name}</td>
                  <td className="px-4 py-3">{s.email}</td>
                  <td className="px-4 py-3"><button className="rounded-lg bg-orange-500 px-3 py-1.5 text-white" onClick={()=> nav(ROUTES.prof.alunoPerfil(s._id || s.id))}>Ver perfil</button></td>
                </tr>
              ))}
