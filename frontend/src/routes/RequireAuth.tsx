...
export default function RequireAuth({ userType }: RequireAuthProps) {
  ...
  const detectedUserType = userType || (location.pathname.startsWith('/aluno') ? 'aluno' : 'professor');
  
  // Se não tem token, redireciona imediatamente para o login correto
  const token = localStorage.getItem(STORAGE_TOKEN_KEY);
  if (!token) {
    const loginRoute = detectedUserType === 'aluno' ? ROUTES.auth.loginAluno : ROUTES.auth.loginProf;
  ...
  if (detectedUserType === 'aluno' && state.role !== 'aluno') {
    return <Navigate to={ROUTES.auth.loginAluno} replace state={{ from: location }} />;
  }
  
  if (detectedUserType === 'professor' && state.role !== 'professor') {
    return <Navigate to={ROUTES.auth.loginProf} replace state={{ from: location }} />;
  }
